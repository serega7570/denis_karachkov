...
class AppModel {
  static async getTasklists() {
    ...
    static async addTasklist(tasklistName, tasklistTime, tasklistPunkt) {
        console.log(JSON.stringify({ tasklistName, tasklistTime, tasklistPunkt }));
    const result = await fetch(
      'http://localhost:4321/tasklists',
      {
        ...
        body: JSON.stringify({ tasklistName, tasklistTime, tasklistPunkt })
      }
    );
...
class App {
  ...
  onInputKeydown = async ({ key, target }) => {
    if (key === 'Enter') {
      if (target.value) {
        await AppModel.addTasklist(target.value, time.value, punkt.value);
  ...
  async init() {
    ...
    tasklists.forEach(({ tasklistName, tasklistTime, tasklistPunkt, tasks }) => {
      const newTasklist = new Tasklist({
        tlName: tasklistName,
        tlTime: tasklistTime,
        tlPunkt: tasklistPunkt,
        tlID: `TL${this.tasklists.length}`,
        moveTask: this.moveTask
      });
...
class Tasklist {
  ...
  onAddTaskButtonClick = async () => {
    const newTaskName = prompt('Введите ФИО');
  ...
  onEditTask = async (taskID) => {
    ...
    const newTaskName = prompt('Введите новое ФИО', oldTaskName);
  ...
  onDeleteTaskButtonClick = async (taskID) => {
    ...
    if (!confirm(`Зритель '${taskName}' будет удален. Продолжить?`)) return;
  ...
  render() {
    ...
    header.innerHTML = 'Свободных мест:<br>' + this.tlName;
    tasklist.appendChild(header);

    const time = document.createElement('header');
    time.classList.add('tm-tasklist-header');
    time.innerHTML = 'Время начала:<br>' + this.tlTime;
    tasklist.appendChild(time);

    const punkt = document.createElement('header');
    punkt.classList.add('tm-tasklist-header');
    punkt.innerHTML = 'Название фильма:<br>' + this.tlPunkt;
    tasklist.appendChild(punkt);
    ...
    button.innerHTML = 'Добавить зрителя';
